...
function UsagePage() {
  ...
  return (
    <AppShell user={session.data} title="用量">
      <SettingsSection
        title="智能体排行榜"
        description="按智能体代码行数排序，其次按已创建 PR 数和智能体运行次数排序。"
        action={
          <Select
            value={activePeriod}
            onValueChange={(value) =>
      ...
      >
        {leaderboard.isLoading || usageIsPrecomputing ? (
          ...
          <p className="p-4 text-xs text-destructive">
            加载用量数据失败：{leaderboard.error.message}
          </p>
        ) : !leaderboard.data?.rows.length ? (
          <div className="p-6 text-center text-sm text-muted-foreground">
            {PERIOD_LABELS[activePeriod].toLowerCase()} 还没有记录 LX-AICODING 用量。
          </div>
        ) : (
          <UsageTable
      ...
      <SettingsSection
        ...
        description="Issues surfaced by PR Review and how often users addressed them."
      >
