...
function formatToolDisplay(
  ...
  switch (toolKind) {
    ...
    case "search": {
      ...
      if (query) {
        return `搜索("${query.slice(0, 40)}${query.length > 40 ? "..." : ""}")`;
      }
      return title;
    }
    case "fetch": {
      if (url) {
        return `获取(${url.slice(0, 50)}${url.length > 50 ? "..." : ""})`;
      }
      return title;
    }
    case "execute": {
      ...
      <div className="my-0.5 text-[12px] leading-5">
        ...
        >
          <span className={isError ? "text-red-400" : "text-[color:var(--ui-text-muted)]"}>
            已编辑 <span className="text-[color:var(--ui-accent)]">{fileName}</span>
          </span>
        </button>
      </div>
    );
...
export const ToolExecution = memo(function ToolExecution({
  ...
  if (isEditOp && status === "pending" && diffData) {
    return (
      <div className="my-1 text-[12px] leading-5">
        ...
        <span className="text-[color:var(--ui-text-dim)]">等待批准...</span>
      </div>
    );
  }
  ...
  if (isEditOp && status === "in_progress") {
    ...
    return (
      <div className="my-0.5 text-[12px] leading-5">
        <span className="text-yellow-400">正在编辑 {getFileName(path)}...</span>
      </div>
    );
  }
  ...
  return (
    <div className="my-0.5 text-[12px] leading-5">
      ...
      {status !== "error" && output && (toolKind === "search" || toolKind === "read" || toolKind === "fetch") && (
        <div className="mt-1">
          <button
            type="button"
            onClick={() => setDetailsExpanded((prev) => !prev)}
            className="text-[11px] text-[color:var(--ui-text-dim)] hover:text-[color:var(--ui-text-muted)]"
          >
            {detailsExpanded ? "隐藏细节" : "显示细节"}
          </button>
          {detailsExpanded && (
            <pre className="mt-1 max-h-48 overflow-auto whitespace-pre-wrap rounded-md bg-[var(--ui-accent-bubble)] px-2 py-1 text-[11px] leading-5 text-[color:var(--ui-text-muted)]">
              {output}
            </pre>
          )}
        </div>
