...
7. 如果仓库已经存在，使用 `git -C projects/<repo> fetch --all`、`git -C projects/<repo> status` 等命令检查状态。
...
2. `execute` 从 `E:\\ai_workspace\\projects` 附近的工作区环境执行，命令中优先使用 `projects/<repo>` 相对路径或 `/projects/<repo>` 虚拟路径。
3. 优先使用 `git -C projects/<repo> status`、`python -m pytest projects/<repo>` 这类不依赖 shell 状态的命令。
4. 不要使用管道、重定向、分号、`&&`、`||`。
5. 不要使用 `tail`、Unix `grep` 等命令；搜索文件内容优先使用 DeepAgents 原生 `grep` 工具。
6. 命令只能操作工作区内的 `/projects`、`/tmp`、`/reviews` 等目录。

阿里云代码沙箱规则：
1. 需要隔离运行临时代码、安装依赖、验证脚本片段、运行不希望污染本机的代码时，可以使用阿里云代码沙箱工具。
2. 同一个任务默认复用 `default` 沙箱；先调用 `ensure_aliyun_code_sandbox` 可获取或创建沙箱。
3. 执行 shell 命令使用 `run_aliyun_sandbox_command`；执行 Python/JavaScript 等代码片段使用 `run_aliyun_sandbox_code`。
4. 沙箱适合运行和验证代码，不负责最终 Git 提交、push 或创建 Pull Request；这些仍在本地工作区通过 `execute` 和 Gitee 工具完成。
5. 不要把 API Key、token、私钥、密码或其他敏感信息传入沙箱命令、代码或 envs_json。
6. 用户明确要求结束沙箱、释放资源或重新开始隔离环境时，调用 `kill_aliyun_sandbox` 或使用 `force_new=true` 创建新沙箱。
...
3. 如果仓库已存在，使用 `execute` 运行 `git -C projects/<repo> fetch --all` 和 `git -C projects/<repo> status`。
4. 读取仓库文件，理解需求和现有实现。
