...
from agent.core.observability import langfuse_agent_run
...
def _tool_kind(tool_name: str) -> str:
    ...
    if tool_name in {"ls", "list_files", "glob", "grep"}:
        return "search"
    if tool_name in {"sync_gitee_repo", "create_pull_request", "open_gitee_pull_request"}:
        return "fetch"
...
def _record_write_todos(thread_id: str, tool_call: Any, index: int) -> bool:
...
def _record_todos(thread_id: str, call_id: str, todos: list[dict[str, str]], *, status: str) -> None:
    """写入结构化任务计划事件。"""
...
def _record_stream_message(thread_id: str, text: str) -> None:
    """把累计正文写成前端可展示的临时文本事件。"""
...
def _record_subagent(thread_id: str, subagent: Any, index: int) -> None:
...
def _consume_interleaved_stream(*, stream: Any, thread_id: str) -> tuple[int, int]:
    ...
    for name, item in stream.interleave("messages", "tool_calls", "subagents"):
        if name == "messages":
            ...
            if text and text != last_message_text:
                ...
                _record_stream_message(thread_id, accumulated_message_text)
        elif name == "tool_calls":
            tool_call_index += 1
            if _record_write_todos(thread_id, item, tool_call_index):
                continue
            logger.debug("忽略官方 tool_call 展示事件：thread_id=%s index=%s item=%s", thread_id, tool_call_index, item)
        elif name == "subagents":
            subagent_index += 1
            _record_subagent(thread_id, item, subagent_index)
    return tool_call_index, subagent_index


def _consume_raw_event_stream(*, stream: Any, thread_id: str, task_kind: str | None = None) -> tuple[int, int]:
    ...
    for event in stream:
            ...
            if tool_name == "write_todos":
                ...
                if isinstance(args, dict):
                    ...
                    if payload_text != write_todo_last_payload_by_call.get(call_id):
                        _record_todos(thread_id, call_id, todos, status="completed")
                    ...
                    if todos and payload_text != write_todo_last_payload_by_call.get(call_id):
                        _record_todos(thread_id, call_id, todos, status="in_progress")
                        write_todo_last_payload_by_call[call_id] = payload_text
            continue
        ...
        if tool_call is not None:
            tool_call_index += 1
            if _record_write_todos(thread_id, tool_call, tool_call_index):
                saw_write_todos = True
                continue
            logger.debug(
            ...
            if tool_name == "write_todos":
                ...
                if isinstance(tool_input, dict):
                    ...
                    if todos and payload_text != write_todo_last_payload_by_call.get(call_id):
                        ...
                        _record_todos(thread_id, call_id, todos, status=event_status)
                        write_todo_last_payload_by_call[call_id] = payload_text
            continue
        ...
        if subagent is not None:
            subagent_index += 1
            _record_subagent(thread_id, subagent, subagent_index)
        ...
        stream = agent.stream_events(
            ...
            config={
                ...
                "metadata": {
                    ...
                    "repo_url": mask_token(repo_url or ""),
                },
                "tags": [tag for tag in ["lx-aicoding", task_kind] if tag],
                "run_name": "lx-aicoding-agent-run",
            },
        )
