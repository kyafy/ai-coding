...
from agent.api.schemas import TaskCreateRequest
...
def backend_logs(limit: int = 200, date: str | None = None) -> dict[str, Any]:
    """读取后端日志最近若干行。

    前端可以轮询这个接口展示服务运行状态；
    讲课时也可以直接在浏览器中打开，观察请求和错误日志。
    date 参数用于读取历史日志，例如 2026-06-22。
    """
